...
impl APCMini {
    ...
    pub fn new(tx: Sender<Vec<u8>>) -> Result<APCMini> {
        ...
        let port_out = Self::find_apcmini_output(&midi_out).ok_or("cannot find APCMini output port")?;
        let port_in = Self::find_apcmini_input(&midi_in).ok_or("cannot find APCMini input port")?;
